...
beta_vals_range = (0.001, 0.9)
doubling_times_range = (0.25, 15)
starting_pops = [1000, 1000, 1000, 1000, 1000]
# Define momentum-related variables
momentum_weight = 0.3  # Adjust the momentum weight as needed
previous_best_config = None

# Perform the randomized grid search with momentum for doubling times and beta values
num_runs = 10000  # Number of randomized search runs
...
def simulate_run(treatment_days, alpha, beta_vals, starting_pops, doubling_times, days_of_treatment):
    pop_arr = []
    normalizer_input = []
    dose_plan_g = [[4, 4, 4, 4], [1, 3, 5, 7], [7, 5, 3, 1]]
    for dose_plan in dose_plan_g:
        for enum, beta in enumerate(beta_vals):
            dose_counter = 0
            total_pop = 0
            for day in range(days_of_treatment):
                population = starting_pops[enum]
                dose = 0
                if population < 1:
                ...
                if day in treatment_days:
                    ...
                    dose_counter += 1
                if doubling_times[enum] != 0 and day % doubling_times[enum] == 0:
                    population = max(0, starting_pops[enum] * 2)  # Ensure non-negativity
...
for _ in tqdm(range(num_runs)):
    # Initialize doubling times and beta values
    if previous_best_config is not None and random.random() < momentum_weight:
        ...
        doubling_times = [max(0, val + random.randint(-1, 1)) for val in previous_best_config[1]]  # Ensure non-negativity
    else:
        # Randomly sample new doubling times and beta values
        beta_vals = [max(0, random.uniform(*beta_vals_range)) for _ in range(5)]  # Ensure non-negativity
        doubling_times = [max(0, random.uniform(*doubling_times_range)) for _ in range(5)]  # Ensure non-negativity

    key = (tuple(beta_vals), tuple(doubling_times))
...
print(
    f"Beta Vals: {best_config[0]}, Doubling Times: {best_config[1]}, Cosine Similarity Score: {best_score}")
