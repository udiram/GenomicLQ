...
alpha_range = (0.1, 0.3)  # Range for alpha optimization
...
def simulate_run(treatment_days, alpha, beta_sub_pops, starting_sub_pops, doubling_sub_pops, days_of_treatment):
    ...
    css = cosine_similarity_score([1, 5, 56], normalized)
    return css
...
for _ in tqdm(range(num_runs)):
    ...
    if previous_best_config is not None and random.random() < momentum_weight:
        ...
        beta_vals = [val + random.uniform(-0.1, 0.1) for val in previous_best_config[0]]
        ...
        alpha = max(alpha_range[0], alpha + random.uniform(-0.05, 0.05))  # Adjust alpha with perturbations
    else:
        ...
        alpha = random.uniform(*alpha_range)  # Randomly sample alpha within the specified range

    key = (tuple(beta_vals), tuple(starting_pops), tuple(doubling_times), alpha)
...
print(f"Cosine Similarity Score: {best_score}, Beta Vals: {best_config[0]}, Starting Pops: {best_config[1]}, Doubling Times: {best_config[2]}, Alpha: {best_config[3]}")
